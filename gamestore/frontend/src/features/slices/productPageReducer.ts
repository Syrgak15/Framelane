...
export const fetchProductBySlug = createAsyncThunk<Product, string>(
    'fetchBySlug',
    async (slug) => {
        const req = await fetch(`${process.env.API_URL}/products/${slug}`);
...
export const postProductReview = createAsyncThunk<ProductReview, PostProductReviewArgs>(
    ...
    async ({ slug, data, token }, { rejectWithValue }) => {
        try {
            const res = await fetch(`${process.env.API_URL}/reviews/${slug}`, {
                method: 'POST',
                headers: {
...
>(
    ...
    async ({ token  }, { rejectWithValue }) => {
        try {
            const res = await fetch(`${process.env.API_URL}/wishlist/all`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`,
