...
export const fetchProductBySlug = createAsyncThunk<Product, string>(
    'fetchBySlug',
    async (slug) => {
        const req = await fetch(`https://framelane-2.onrender.com/products/${slug}`);
...
export const postProductReview = createAsyncThunk<ProductReview, PostProductReviewArgs>(
    ...
    async ({ slug, data, token }, { rejectWithValue }) => {
        try {
            const res = await fetch(`https://framelane-2.onrender.com/reviews/${slug}`, {
                method: 'POST',
                headers: {
...
>(
    ...
    async ({ token  }, { rejectWithValue }) => {
        try {
            const res = await fetch(`https://framelane-2.onrender.com/wishlist/all`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`,
